...
class dedisp_method(object):
    ...
    def total_smear_all_DMs(self):
        return self.total_smear(self.DMs)
...
def dm_steps(loDM, hiDM, obs, cohdm=0.0, numsub=0, numprocs=1,
    ...
    dm_steps(loDM, hiDM, obs, cohdm=0.0, numsub=0, numprocs=1,
        ...
        added option to pass in allow_dDMs
    """
    if allow_dDMs is None:
    ...
    print("Total number of DM trails:", total_numDMs)
...
############### Writing dedisp.py ####################
##### kind of hackey copy-pasting, may not work/be dodgy
##### have not checked
...
for dDM, dsubDM, dmspercall, downsamp, subcall, startDM in zip(dDMs, dsubDMs, dmspercalls, downsamps, subcalls, startDMs):
    ...
    for ii in range(subcall):
        ...
        if outsubs:
            ...
            myexecute("prepsubband -dmprec %d -sub -subdm %.2f -nsub %d -downsamp %d -o %s %s" %
                      (dmprec, subDM, nsub, subdownsamp, basename, filfile))
            # And now create the time series
            subnames = basename+"_DM%.2f.sub[0-9]*"%subDM
            myexecute("prepsubband -dmprec %d -lodm %.3f -dmstep %.3f -numdms %d -downsamp %d -o %s %s" %
                      (dmprec, loDM, dDM, dmspercall, datdownsamp, basename, subnames))
        else:
            myexecute("prepsubband -dmprec %d -nsub %d -lodm %.3f -dmstep %.3f -numdms %d -downsamp %d -o %s %s" %
                      (dmprec, nsub, loDM, dDM, dmspercall, downsamp, basename, filfile))
"""
...
for dDM, dsubDM, dmspercall, downsamp, subcall, startDM in zip(dDMs, dsubDMs, dmspercalls, downsamps, subcalls, startDMs):
    ...
    for ii in range(subcall):
        ...
        if outsubs:
            ...
            if downsamp < 2: subdownsamp = datdownsamp = 1
            # First create the subbands
            myexecute("prepsubband -dmprec %d -sub -subdm %.2f -nsub %d -downsamp %d -mask %s_rfifind.mask -o %s %s" %
                      (dmprec, subDM, nsub, subdownsamp, basename, basename, filfile))
            # And now create the time series
            subnames = basename+"_DM%.2f.sub[0-9]*"%subDM
            myexecute("prepsubband -dmprec %d -lodm %.3f -dmstep %.3f -numdms %d -downsamp %d -o %s %s" %
                      (dmprec, loDM, dDM, dmspercall, datdownsamp, basename, subnames))
        else:
            myexecute("prepsubband -dmprec %d -nsub %d -lodm %.3f -dmstep %.3f -numdms %d -downsamp %d -mask %s_rfifind.mask -o %s %s" %
                      (dmprec, nsub, loDM, dDM, dmspercall, downsamp, basename, basename, filfile))
"""
# not having -nsub in the call just makes it default to 32
dedisp_template2_nosubbanding_withmask = """
# No subbanding
# Loop over the DDplan plans
for dDM, dmpermethod, downsamp, startDM in zip(dDMs, dmspermethods, downsamps, startDMs):
    myexecute("prepsubband -dmprec %d -nsub %d -lodm %.3f -dmstep %.3f -numdms %d -downsamp %d -mask %s_rfifind.mask -o %s %s" %
                      (dmprec, nchan, startDM, dDM, dmpermethod, downsamp, basename, basename, filfile))
"""
...
for dDM, dmpermethod, downsamp, startDM in zip(dDMs, dmspermethods, downsamps, startDMs):
    myexecute("prepsubband -dmprec %d -nsub %d -lodm %.3f -dmstep %.3f -numdms %d -downsamp %d -o %s %s" %
                      (dmprec, nchan, startDM, dDM, dmpermethod, downsamp, basename, filfile))
"""

def select_correct_dedisp_template2(subbanding, mask):
    if subbanding:
        if mask:
            return dedisp_template2_with_mask
        else:
            return dedisp_template2
    else:
        if mask:
            return dedisp_template2_nosubbanding_withmask
        else:
            return dedisp_template2_nosubbanding
        
        
def extract_dedisp_params(methods):
    """returns:
    dDMs, startDMs, downsamps, nsub, dsubDMs, dmspercalls, subcalls, dmspermethods, subbanding, nchan, dmprec"""
    dDMs = [m.dDM for m in methods]
    startDMs = [m.loDM for m in methods]
    downsamps = [m.downsamp for m in methods]
     # set numsubbands and check same for all methods
    numsubbands = methods[0].numsub
    for m in methods:
        if m.numsub != numsubbands:
            raise AttributeError("Number of subbands is not the same for all methods")
    dsubDMs = [m.dsubDM for m in methods]
    dmspercalls = [m.DMs_per_prepsub for m in methods]
    subcalls = [m.numprepsub for m in methods]
    dmspermethods = [len(m.DMs) for m in methods]
    subbanding = numsubbands != 0
    nchan = methods[0].obs.numchan
    # find dm precision
    dmprec = 1
    for ddm in dDMs:
        while ddm * 10**dmprec // 1 == 0:
            dmprec +=1
    return dDMs, startDMs, downsamps, numsubbands, dsubDMs, dmspercalls, subcalls, dmspermethods, subbanding, nchan, dmprec
...
def print_dedisp_methods(methods, filfile, mask=True):
    """
    ...
    """
    dDMs, startDMs, downsamps, numsubbands, dsubDMs, dmspercalls, subcalls, dmspermethods, subbanding, nchan, dmprec = extract_dedisp_params(methods)
    basename, ext = os.path.splitext(filfile)
    
    startdmstring = "["
    for x in startDMs:
        startdmstring += f"{x:.{dmprec}f}, "
    startdmstring = startdmstring[:-2] + "]"  # take off the last ", "
    
    print(dedisp_template1)
    print(f"nsub = {numsubbands}\n\n")
    print(f"nchan = {nchan}\n")
    print(f"dmprec = {dmprec}\n")
    print(f"basename = {repr(basename)}\n")
    print(f"filfile = {repr(filfile)}\n\n")
...
subcalls    = {subcalls}\n""")
    # had to finagle this to avoid 0.435000000000002-esque numbers
    print(f"""# The low DM for each set of DMs
startDMs    = {startdmstring}\n""")
    if subbanding:
        print(f"""# DMs/call
dmspercalls = {dmspercalls}\n""")
    else:
        print(f"""# DMs/plan
dmspermethods = {dmspermethods}\n""")
        
    print(select_correct_dedisp_template2(subbanding, mask))  
    
def write_dedisp_methods_to_file(methods, filfile, mask=True):
    """
    ...
    with open('dedisp_%s.py'%basename, 'w') as f:
        ...
        f.write(f"dmprec = {dmprec}\n")
        f.write(f"basename = {repr(basename)}\n")
        f.write(f"filfile = {repr(filfile)}\n\n")

        f.write(f"""# dDM steps from DDplan.py
dDMs        = {dDMs}\n""")
        if subbanding:
            f.write(f"""# dsubDM steps
dsubDMs     = {dsubDMs}\n""")
...
subcalls    = {subcalls}\n""")
        # finagled this to avoid 0.435000000000002-esque numbers
...
dmspermethods = {dmspermethods}\n""")
        ...
        f.write(select_correct_dedisp_template2(subbanding, mask))
        

############### Write the commands themselves ####################

def split_into_fewer_numdms_per_call(cmd, max_numdms):
    type_conv = {
        "-dmprec": int,
        "-nsub": int,
        "-lodm": float,
    ...
    while i <len(spltcmd):
        if spltcmd[i][0] == "-":
            opt = spltcmd[i]
            opts[opt] = type_conv[opt](spltcmd[i+1])
            i += 2
        else:
            i += 1
        
    orig_lodm = opts.pop('-lodm')
    orig_numdms = opts.pop('-numdms')
    
    # if already less than max just print original command
    if orig_numdms <= max_numdms:
        return(cmd)
    
    # find lowest number can divide numdms by which will give an integer <= max_numdms
    divider = min([n for n in np.arange(1, 260) if ((orig_numdms%n==0) and (orig_numdms//n <= max_numdms))])
    new_numdms = orig_numdms//divider
    
    cmd = ""
    for j in range(divider):
        lodm = round(orig_lodm + j*opts['-dmstep']*new_numdms, opts['-dmprec'])
        command = f"prepsubband -lodm {lodm} -numdms {new_numdms}"
        for key in list(opts.keys()):
            command += f" {key} {opts[key]}"
        command += f" {fil}"
        cmd += command + "\n"
    return cmd[:-1]
    

def print_prepsubband_commands(methods, filfile, subdir="", outbasename="", outsubs=False, max_dms_per_cal=104, mask=True, otheropts=""):
    dDMs, startDMs, downsamps, nsub, dsubDMs, dmspercalls, subcalls, dmspermethods, subbanding, nchan, dmprec = extract_dedisp_params(methods)
    basename, ext = os.path.splitext(filfile)
    if subdir:
        filfile = os.path.join(subdir,filfile)
        basename = os.path.join(subdir,basename)
        if outbasename:
            outbasename = os.path.join(subdir, outbasename)
    if not outbasename:
        outbasename=basename
        
    if mask:
        prepsub_opts = f"-mask {basename}_rfifind.mask {otheropts}"
    else:
        prepsub_opts = otheropts
    ...
    for dDM, dsubDM, dmspercall, downsamp, subcall, startDM in zip(dDMs, dsubDMs, dmspercalls, downsamps, subcalls, startDMs):
        ...
        for ii in range(subcall):
            ...
            if outsubs:
                ...
                if downsamp < 2: subdownsamp = datdownsamp = 1
                # First create the subbands
                print(f"prepsubband -dmprec {dmprec:d} -sub -subdm{subDM:.{dmprec}f} -nsub {nsub:d} -downsamp {subdownsamp:d} {prepsub_opts} -o {outbasename} {filfile}")
                # And now create the time series
                subnames = basename+"_DM%.2f.sub[0-9]*"%subDM
                print(f"prepsubband -dmprec {dmprec:d} -lodm {loDM:.{dmprec}f} -dmstep {dDM:.{dmprec}f} -numdms {dmspercall} -downsamp {datdownsamp} {otheropts} -o {outbasename} {subnames}") 
            else:
                cmd=f"prepsubband -dmprec {dmprec:d} -nsub {nsub:d} -lodm {loDM:.{dmprec}f} -dmstep {dDM:.{dmprec}f} -numdms {dmspercall:d} -downsamp {downsamp:d} {prepsub_opts} -o {outbasename} {filfile}"
                print(split_into_fewer_numdms_per_call(cmd, max_dms_per_cal))

def print_prepsubband_commands_predownsampled(methods, filfile, subdir="", outbasename="", outsubs=False, max_dms_per_cal=104):
    dDMs, startDMs, downsamps, nsub, dsubDMs, dmspercalls, subcalls, dmspermethods, subbanding, nchan, dmprec = extract_dedisp_params(methods)
    """Same as print_prepsubband_commands, but assumes the files have already been downsampled.
    If you have downsamp of 4 in a method and your original file was x_pow.fil, it assumes a x_pow_t4.fil file exists"""
    basename, ext = os.path.splitext(filfile)
    if subdir:
        filfile = os.path.join(subdir,filfile)
        basename = os.path.join(subdir,basename)
    ...
    for dDM, dsubDM, dmspercall, downsamp, subcall, startDM in zip(dDMs, dsubDMs, dmspercalls, downsamps, subcalls, startDMs):
        ...
        for ii in range(subcall):
            ...
            if outsubs:
                # Get our downsampling right
                print("No idea what's going on for subband downsampling so I skipped this")
            else:
                cmd=f"prepsubband -dmprec {dmprec:d} -nsub {nsub:d} -lodm {loDM:.{dmprec}f} -dmstep {dDM:.{dmprec}f} -numdms {dmspercall:d} -mask {basename}_rfifind.mask -o {outbasename} {basename}_t{downsamp:d}.fil"
    
                print(split_into_fewer_numdms_per_call(cmd, max_dms_per_cal))
                
######################################################
    
def plot_many_methods(methods, figsize=(12,10)):
    fig, ax = plt.subplots(figsize=figsize)
    
    work_fracts_tmp = [meth.numDMs/float(meth.downsamp) for meth in methods]
    work_fracts =  np.asarray(work_fracts_tmp)/sum(work_fracts_tmp)

    for ddp, work_fract in zip(methods, work_fracts):
        ddp.plot(work_fract, axs=ax)

    ax.set_yscale("log")
    ax.set_ylim([2E-2,None])
    ax.set_xlabel("DM")
    ax.set_ylabel("smearing (ms)")
    plt.tight_layout()
    return fig, ax
...
class ddplan:
    ...
    @property
    def work_fracts(self):
        work_fracts = [meth.numDMs/float(meth.downsamp) for meth in self.methods]
        return np.asarray(work_fracts)/sum(work_fracts)
    
    def __str__(self):
        if (self.methods[0].numsub):
            outstr = "  Low DM    High DM     dDM  DownSamp  dsubDM   #DMs  DMs/call  calls  WorkFract\n"
        else:
            outstr = "  Low DM    High DM     dDM  DownSamp   #DMs  WorkFract\n"

        for method, fract in zip(self.methods, self.work_fracts):
            outstr += str(method) + "  %.4g" % fract + "\n"
        return outstr


# Not tested yet
# Wrote in a rush, quite clunky
@attrs 
class ddplanCollection():
    """Class to store a collection of ddplans, e.g. if have many coherent DMs and a ddplan for each"""
    ddplans: list = attrib(factory=list)
    
    @property
    def cdms(self):
        return list(self.indexed_by_cdm.keys())
    
    @property
    def indexed_by_cdm(self):
        """Returns a {cdm: ddplan} dictionary"""
        out = {}
        for ddp in self.ddplans:
            out[ddp.methods[0].obs.cDM] = ddp
        return out
    ...
    def all_methods(self):
        all_meth = []
        for ddp in self.ddplans:
            all_meth.extend(ddp.methods)
        return all_meth
    
    def __str__(self):
        outstr = ""
        for ddp in self.ddplans:
            outstr += f"coherent DM: {ddp.methods[0].obs.cDM}\n"
            outsr += str(ddp) + "\n"
        return outstr
    
    def write_to_npz(self, filename):
        """Save ddplanCollection to npz file - UNTESTED"""
        cdm_dict = self.indexed_by_cdm
        cdms = list(cdm_dict.keys())
        save_dict = dict(cdms=cdms)
        
        i = 0
        for cdm in cdms:
            method_dicts = []
            for method in cdm_dict[cdm].methods:
                method_dicts.append(method.as_dict())
            save_dict[i] = method_dicts
            i += 1
            
        np.savez(filename, **save_dict)
        print(f"Wrote {len(cdms)} ddplans to {filename}")

    @classmethod
    def read_from_npz(cls, filename):
        """Read ddplanCollection from npz file - UNTESTED"""
        data = np.load(filename, allow_pickle=True)
        cdms = data['cdms']
        ddplans = []
        i = 0
        for cdm in cdms:
            methods = []
            for method_dict in data[i]:
                methods.append(dedisp_method.from_dict(method_dict))
            ddplans.append(ddplan(methods))
        print(f"Read {len(ddplans)} ddplans from {filename}")
        return cls(ddplans=ddplans)
...
class frankenstein_ddplan(ddplan):
    ...
    def make(cls, ddplan_methods, max_meth, hi_DM, troubleshoot=False, extend_up_to=0):
        ...
        """
        print("DO NOT USE!!! THIS IS BROKEN. Something in dedisp_method.make")
        return cls(methods=[], methods_by_cdm={})
    
        # NB assumes lo_DM is 0
        base_methods = ddplan_methods[0:max_meth]
        half_span_per_cDM = base_methods[-1].hiDM
        
        numsubbands = ddplan_methods[0].numsub
        numprocs = ddplan_methods[0].numprocs
        include_scattering = ddplan_methods[0].include_scattering

        if extend_up_to == 0 or extend_up_to < hi_DM:
            extend_up_to = hi_DM
        ...
        while cDM <= hi_DM:
            ...
            print_message(f"### cDM = {cDM} ###")

            print_message("## LEFT ##")
            for i in range(max_meth)[::-1]:
                curr_meth = ddplan_methods[i]
                print_message(f"i={i}")
                print_message(f"cDM = {cDM}, curr_meth.hiDM = {curr_meth.hiDM}, new_lo = {cDM - curr_meth.hiDM}")
                print_message(f"curr_meth.loDM = {curr_meth.loDM}, hi_DM = {hi_DM}, cDM - curr_meth.loDM = {cDM - curr_meth.loDM}")
                new_lo = cDM - curr_meth.hiDM
                new_hi = min(cDM - curr_meth.loDM, hi_DM)
                print_message(f"new_hi = {new_hi}")
                ...
                print_message(f"{i}: {new_meth.loDM} -> {new_meth.hiDM} in steps of {new_meth.dDM} (cdm={new_meth.obs.cDM})")
                amalgamated_ddplan_methods.append(new_meth)
                cDM_plans.append(new_meth)

            print_message("## RIGHT ##")
            # always want to be in the right part of the arc when you hit the end of the range
    ...
    def plot(self, figsize=(12,10), make_title=False):
        fig, ax = plt.subplots(figsize=figsize)

        for ddp, work_frac in zip(self.methods, self.work_fracts):
            ddp.plot(work_frac, axs=ax)
