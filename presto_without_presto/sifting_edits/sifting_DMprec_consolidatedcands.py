...
class Candidate(object):
    ...
    def __str__(self):
        ...
        out_string = (
            f"{cand:<65s}   {self.DM:{dmprec+5}.{dmprec}f}  {self.snr:6.2f}  {self.sigma:6.2f}  "
            f"{'%2d'.center(7)%self.numharm}  {self.ipow_det:7.1f}  {self.cpow:7.1f}  "
            f"{self.p*1000:12.6f}  {self.r:10.2f}  {self.z:8.2f} "
        )
        return out_string
...
class Candlist(object):
    ...
    def plot_summary(self, usefreqs=True):
        ...
        cb = plt.colorbar(
            scatt, cax=cax, ticks=(0, 1, 2, 3, 4), format=fmtr, orientation="horizontal"
        )
        ...
        if not usefreqs:
            plt.gca().xaxis.set_ticks(
                np.concatenate(
                    (np.logspace(-4, 0, 4, endpoint=False), np.linspace(1, 15, 8))
                )
            )
            ...
            plt.xlim(
                max(short_period / 5.0, min(xdata) / 5.0),
    ...
    def plot_rejects(self, usefreqs=True):
        ...
        candlists = [
            ...
            self.badlists["longperiod"],
            self.badlists["shortperiod"],
            self.badlists["threshold"],
            self.badlists["harmpowcutoff"],
            self.badlists["rogueharmpow"],
            self.badlists["harmonic"],
        ...
        handles = []
        for cands, colour, marker, zorder, size, fixedsize, lw in zip(
            candlists, colours, markers, zorders, sizes, fixedsizes, lws
        ):
            if len(cands):
            ...
            handles.append(
                plt.scatter([0], [0], s=size, c=colour, marker=marker, alpha=0.7)
            )

        fig.legend(handles, labels, "lower center", prop={"size": "x-small"}, ncol=4)
        ...
        if not usefreqs:
            ...
            plt.xlim(
                max(short_period / 5.0, min(all_xdata) / 5.0),
    ...
    def plot_goodcands(self, usefreqs=True):
        ...
        handles = []
        for cands, colour, marker, zorder, size, fixedsize, lw, alpha, ec in zip(
            candlists, colours, markers, zorders, sizes, fixedsizes, lws, alphas, ecs
        ):
            sigmas = []
            dms = []
            xdata = []
            ...
            handles.append(
                plt.scatter([], [], s=size, c=colour, marker=marker, alpha=0.7)
            )
        ...
        for ii in reversed(list(range(len(self.cands)))):
            ...
            for bird, err in known_birds_f:
                if np.fabs(cand.f - bird) < err:
                    known_bird = 1
                    cand.note = (
                        "Freq (%.2f Hz) is within %g Hz "
                        "of a known birdie centred at %.2f Hz" % (cand.f, err, bird)
                    )
                    break
            if known_bird:
    ...
    def reject_threshold(self, sigma_threshold=None, c_pow_threshold=None):
        ...
        for ii in reversed(list(range(len(self.cands)))):
            ...
            if cand.numharm == 1:
                ...
                if (cand.sigma < sigma_threshold) and (cand.cpow < c_pow_threshold):
                    cand.note = (
                        "Only 1 harmonic and both sigma "
                        "(%g < %g) and coherent power (%g < %g) are "
                        "too low."
                        % (cand.sigma, sigma_threshold, cand.cpow, c_pow_threshold)
                    )
                    self.mark_as_bad(ii, "threshold")
            else:
                # Multiple harmonic case
                if cand.sigma < sigma_threshold:
                    cand.note = "%d harmonics and sigma " "(%g < %g) is too low." % (
                        cand.numharm,
                        cand.sigma,
                        sigma_threshold,
                    )
    ...
    def remove_duplicate_candidates(self, verbosity=1):
        ...
        """
        if verbosity >= 1:
            print("  Sorting the %d candidates by frequency..." % self.get_numcands())
        ...
        while ii < self.get_numcands():
            jj = ii + 1
            if (
                jj < self.get_numcands()
                and np.fabs(self.cands[ii].r - self.cands[jj].r) < r_err
            ):
            ...
            while 1:
                ...
                for factor in np.arange(1.0, 17.0):
                    ...
                    if zapj:
                        if verbosity >= 2:
                            print(
                                "Removing %s:%d (%.2f Hz) because it is "
                ...
                for numer, denom in zip(
                    [3.0, 5.0, 2.0, 4.0, 5.0, 3.0, 5.0, 2.0, 3.0, 4.0],
                    [2.0, 2.0, 3.0, 3.0, 3.0, 4.0, 4.0, 5.0, 5.0, 5.0],
                ):
                    factor = numer / denom
                    if np.fabs(fundcand.f - harmcand.f * factor) < f_err * factor:
                        if verbosity >= 2:
                ...
                if zapj:
                    harmcand.note = (
                        f"This candidate (P={harmcand.p:.4f} s, DM={harmcand.DM:.{dmprec}f}) is "
                        f"a harmonic ({harmstr}) of {fundcand.filename}:{fundcand.candnum:d} "
                        f"(P={fundcand.p:.4f} s, DM={fundcand.DM:.{dmprec}f})."
                    )
                    numremoved += 1
    ...
    def remove_DM_problems(self, numdms, dmlist, low_DM_cutoff, verbosity=1, dm_iqr_limit=None):
        ...
        dmstrs = [f"{dm:.{dmprec}f}" for dm in dms]
        ...
        for ii in reversed(list(range(len(self.cands)))):
            ...
            if len(currcand.hits) < numdms:
                ...
                num_toofew += 1
                currcand.note = (
                    "Candidate has only %d DM hits. This is less "
            ...
            if float(hitdm) <= low_DM_cutoff:
                ...
                num_toolow += 1
                currcand.note = (
                    f"Hit with max sigma ({hitsigma:g}) has dm ({hitdm:.{dmprec}f}) "
                    f"<= low DM cutoff ({low_DM_cutoff:.{dmprec}f}) "
                )
                self.mark_as_bad(ii, "dmproblem")
                if verbosity >= 2:
                    print(
            ...
            if len(currcand.hits) > 1:
                ...
                dm_indices = np.asarray(
                    [
                        dmdict[f"{currcand.hits[jj][0]:.{dmprec}f}"]
    ...
    def print_cand_summary(self, summaryfilenm=None):
        ...
        summaryfile.write(
            "      # Short period rejects:        %d\n"
            % len(self.badlists["shortperiod"])
        )
        summaryfile.write(
            "      # Long period rejects:         %d\n"
        ...
        summaryfile.write(
            "      # Harmonic candidates:         %d\n" % len(self.badlists["harmonic"])
        )
        summaryfile.write(
            "      # Candidates with DM problems: %d\n"
        ...
        for goodcand in self.cands:
            ...
            if len(goodcand.hits) > 1:
                ...
                for hit in goodcand.hits:
                    ...
                    candfile.write(
                        f"  DM={hit[0]:{dmprec+4}.{dmprec}f} SNR={hit[1]:5.2f} Sigma={hit[2]:5.2f}   "
                        + numstars * "*"
                        + "\n"
                    )
...
def sift_directory(dir, outbasenm):
    ...
    dmstrs = [f"{infodata.infodata(inffn).DM:.{dmprec}f}" for inffn in inffns]
    ...
    print("Read %d candidates from %d files" % (len(lo_accel_cands), len(lo_accel_fns)))
    ...
    print("Read %d candidates from %d files" % (len(hi_accel_cands), len(hi_accel_fns)))
    ...
    class LogLinScale(matplotlib.scale.ScaleBase):
        ...
        class LogLinTransform(matplotlib.transforms.Transform):
            ...
            def transform(self, a):
                ...
                if aa.mask.any():
                    aa[a < self.brk] = (
                        np.ma.log10(a[a < self.brk]) - np.log10(self.brk) + self.brk
                    )
                else:
                    aa[a < self.brk] = (
                        np.log10(a[a < self.brk]) - np.log10(self.brk) + self.brk
                    )
                return aa
        ...
        class InvertedLogLinTransform(matplotlib.transforms.Transform):
            ...
            def transform(self, a):
                aa = a.copy()
                aa[a < self.brk] = np.ma.power(
                    10, a[a < self.brk] - self.brk + np.log10(self.brk)
                )
                return aa
