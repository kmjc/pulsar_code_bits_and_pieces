...
class Candidate(object):
    ...
    def __str__(self):
        ...
        return (
            "%-65s   %7.2f  %6.2f  %6.2f  %s   %7.1f  %7.1f  "
            "%12.6f  %10.2f  %8.2f "
            % (
                cand,
                self.DM,
    ...
    def remove_harmonics(self, verbosity=1):
        ...
        while 1:
            ...
            while 1:
                ...
                if zapj:
                    harmcand.note = (
                        "This candidate (P=%.4f s, DM=%.2f) is "
                        "a harmonic (%s) of %s:%d "
                        "(P=%.4f s, DM=%.2f)."
                        % (
                            harmcand.p,
    ...
    def remove_DM_problems(self, numdms, dmlist, low_DM_cutoff, verbosity=1):
        ...
        for ii in reversed(list(range(len(self.cands)))):
            ...
            if float(hitdm) <= low_DM_cutoff:
                ...
                num_toolow += 1
                currcand.note = (
                    "Hit with max sigma (%g) has dm (%.2f) "
                    "<= low DM cutoff (%.2f) " % (hitsigma, hitdm, low_DM_cutoff)
                )
                self.mark_as_bad(ii, "dmproblem")
                if verbosity >= 2:
                    print(
        ...
        for goodcand in self.cands:
            ...
            if len(goodcand.hits) > 1:
                ...
                for hit in goodcand.hits:
                    ...
                    candfile.write(
                        "  DM=%6.2f SNR=%5.2f Sigma=%5.2f   " % hit
                        + numstars * "*"
                        + "\n"
                    )
