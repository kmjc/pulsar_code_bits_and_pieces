...
class Candidate(object):
    ...
    def __str__(self):
        ...
        out_string = f"{cand:<65s}   {self.DM:{dmprec+5}.{dmprec}f}  {self.snr:6.2f}  {self.sigma:6.2f}  " \
            ...
            while 1:
                ...
                for numer,denom in zip([3.0, 5.0, 2.0, 4.0, 5.0, \
                    ...
                    if np.fabs(fundcand.f-harmcand.f*factor) < f_err*factor:
                        ...
                        break
                if zapj:
                    harmcand.note = f"This candidate (P={harmcand.p:.4f} s, DM={harmcand.DM:.{dmprec}f}) is " \
    ...
    def remove_DM_problems(self, numdms, dmlist, low_DM_cutoff, verbosity=1):
        ...
        for ii in reversed(list(range(len(self.cands)))):
            ...
            if float(hitdm) <= low_DM_cutoff:
                numremoved += 1
                num_toolow += 1
                currcand.note = f"Hit with max sigma ({hitsigma:g}) has dm ({hitdm:.{dmprec}f}) " \
        ...
        for goodcand in self.cands:
            ...
            if (len(goodcand.hits) > 1):
                ...
                for hit in goodcand.hits:
                    ...
                    candfile.write(f"  DM={hit[0]:{dmprec+4}.{dmprec}f} SNR={hit[1]:5.2f} Sigma={hit[2]:5.2f}   " + \
                                    numstars*'*' + '\n')
